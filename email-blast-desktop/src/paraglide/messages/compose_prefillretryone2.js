...
const en_compose_prefillretryone2 = /** @type {(inputs: Compose_Prefillretryone2Inputs) => LocalizedString} */ (i) => {
	return /** @type {LocalizedString} */ (`Retry pre-filled from Logs: ${i?.count} failed recipient, the same template, message, and SMTP.`)
};

const id_compose_prefillretryone2 = /** @type {(inputs: Compose_Prefillretryone2Inputs) => LocalizedString} */ (i) => {
...
const compose_prefillretryone2 = /** @type {((inputs: Compose_Prefillretryone2Inputs, options?: { locale?: "en" | "id" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Compose_Prefillretryone2Inputs, { locale?: "en" | "id" }, {}>} */ ((inputs, options = {}) => {
	...
	if (locale === "id") return id_compose_prefillretryone2(inputs)
	return en_compose_prefillretryone2(inputs)
});
